 ...
 * Time: 11:58 PM
 */
class HOUZEZ_property_taxonomies2 extends WP_Widget {
    ...
    public function __construct() {
        ...
        parent::__construct(
            'houzez_property_taxonomies2', // Base ID
            esc_html__( 'HOUZEZ: Property Taxonomies', 'houzez' ), // Name
            array( 'classname' => 'widget-categories', 'description' => esc_html__( 'Show property type, status, features, cities, states, lifestyle, regions', 'houzez' ), ) // Args
        );
    ...
    public function widget( $args, $instance ) {
        ...
        echo '<div class="widget-body">';
            houzez_property_taxonomies2( $prop_taxonomy, $show_child, $show_count );
        echo '</div>';
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'taxonomy' ) ); ?>"><?php esc_html_e( 'Taxonomy', 'houzez' ); ?>
                <select class="widefat" name="<?php echo esc_attr( $this->get_field_name( 'taxonomy' ) ); ?>">
                    ...
                    <option value="property_lifestyle" <?php echo ($instance['taxonomy'] == 'property_lifestyle') ? ' selected="selected"' : ''; ?>><?php esc_html_e( 'Property Lifestyle', 'houzez' ); ?></option>

                    <option value="property_region" <?php echo ($instance['taxonomy'] == 'property_region') ? ' selected="selected"' : ''; ?>><?php esc_html_e( 'Property Regions', 'houzez' ); ?></option>
                </select>
            </label>
        </p>
        <p>
...
if ( ! function_exists( 'HOUZEZ_property_taxonomies2_loader' ) ) {
    function HOUZEZ_property_taxonomies2_loader (){
        register_widget( 'HOUZEZ_property_taxonomies2' );
    }
    add_action( 'widgets_init', 'HOUZEZ_property_taxonomies2_loader' );
}


function houzez_property_taxonomies2( $taxonomy, $show_child, $show_count ) {
    $terms = get_terms( $taxonomy , array( 'parent'=> 0 ));
    if( !is_wp_error($terms) ) {
        $count = count($terms);
        if ( $count > 0 ){
            show_hierarchical_property_types2( $terms, $taxonomy, $show_child, $show_count );
        }
    }
}

function show_hierarchical_property_types2 ( $terms, $taxonomy, $show_child, $show_count ) {
    $count = count( $terms );
    if ( $count > 0 ){
        ...
        foreach ($terms as $term){
            ...
            if( $show_child ) {
                ...
                if ($child_terms) {
                    show_hierarchical_property_types2( $child_terms, $taxonomy, false, $show_count );
                }
            }
